...
#
#
# We focus on the lambdas computation here, as the transition prob. are not 
# used in the clustering 
# This is the script used to generate the data used in "ClusterConnections.R"
#
#######################################################
...
MapState <- function(a,b){
  ...
  # this version contains only 6 states! ud and du are considered the same 
...
EstimateMarkovChainLambdas_Conn <- function(listtest){

  #fits only the exponential coefficient (the lambdas) of one markov chain
  #Coefficients are computed using the MLE.
  ...
  TransitionVector <-df.test.exp$lambda
  TransitionVector <- setNames(TransitionVector, df.test.exp$Statebegin)
  return(TransitionVector)
}

EstimateSeveralMarkovChainsLambdas_Conn <- function(region1 = 1,region2=1:90,patient1=1,mean=F){

  #fits only the exponential coefficient of several markov chains for a chosen number of patients and regions
  ...
  coeff <- array(0,dim=c(length(region1),length(region2),6,length(patient1)))
  setkey(Nodes,index)
  names <- Nodes$name
  names1 <- names[region1]
  names2 <- names[region2]
  statename <- c("uu","uz","ud","zz","zd","dd")
  dimnames(coeff)<-list(names1,names2,statename,NULL)
  ...
  for(j in 1:length(patient1)){
    for(i in 1:length(region1)){
      for(ii in 1:length(region2)){
        ...
        coeff[i,ii,,j] <- as.array(EasyMatrix[statename])
      }
    }
  }
  if(mean)  coeff <- apply(coeff,c(1,2,3),function(x) mean(x,na.rm=T))
  return(coeff)
}
...
DotPlotClusterLambda_Conn<- function(region1 = 1,region2=1:90,patient1=1){
  ...
  for(i in 1:90){
    ...
    plot.df.temp <- cbind(rep(Nodes[index==i]$name,6),rep(Nodes[index==i]$cluster,6))
...
DotPlotClusterLambdaCut_Conn<- function(region1 = 1,region2=1:90,patient1=1,ncut=4,mean=F){
  ...
  for(i in 1:90){
    ...
    for(j in 1:ncut){
    plot.df.temp <- cbind(rep(Nodes[index==i]$name,6),rep(Nodes[index==i]$cluster,6),rep(j,6))
    if(mean) {
...
MarkovCutLambdas_Conn <- function(ts1,ts2,numberofcut){

  #Fits the lambda of one markov chain for the stationarity analysis. The two time series are cut $numberofcut$ times
  # and a CTMC is fitted for each of the sections, keeping only the lambdas coef.

  #generate the list containing the n cut time series
  ...
  markovcut <- array(0,dim=c(6,numberofcut))
  for(i in 1:numberofcut){
    temp <- EstimateMarkovChainLambdas_Conn(CreateMarkovChainList_Conn(ts1_c[[i]],ts2_c[[i]]))
    markovcut[,i] <- temp[statename]
  }
  dimnames(markovcut) <- list(statename,NULL)
  return(markovcut)
}

EstimateSeveralMarkovChainsLambdasCut_Conn <- function(region1 = 1,region2=1:90,patient1=1,numberofcut,mean=F,coeff){


  #Fits the lambda of several markov chains for the stationarity analysis. The time series are cut $numberofcut$ times
  # and a CTMC are fitted for each of the sections
  # if mean is set to TRUE, returns the mean coefficients across patient. Otherwise returns the full array.
  # also saves the data as .rda for easy retrieving

    # initialize the 6 dimension array to zero and the correct size
   coeff <- array(0,dim=c(length(region1),length(region2),6,length(patient1),numberofcut))
    ...
    for(j in 1:length(patient1)){
      ...
      for(i in 1:length(region1)){
        for(ii in 1:length(region2)){
          cat(".")
          EasyMatrix <- MarkovCutLambdas_Conn(HCP.bin[region1[i],,patient1[j]],HCP.bin[region2[ii],,patient1[j]],numberofcut)
          coeff[i,ii,,j,] <- as.array(EasyMatrix)
        }
      }
      save(coeff, file = 'MarkovChainLambdas4CutCoeffConnections.rda')
      cat("\n")
    }
...
#### EXECUTION EXAMPLES ####
#CompareCoeffWithSingleRegion(HCP.bin[21,,1],HCP.bin[22,,1])
#PlotCoefficientTest(77,"zu")
#MarkovChain4CutCoeffConnections <- EstimateSeveralMarkovChainsLambdasCut_Conn(1:90,1:90,1:90,4,F,coeff)
#DotPlotClusterLambdaCut_Conn(5,1:90,1,4,F)
#FitExp_Conn(3,49,1,"zd")
#summ <- GetSummarydf(CreateMarkovChainList_Conn(HCP.bin[3,,1],HCP.bin[49,,1]))
#testm <-TestMarkov(CreateMarkovChainList_Conn(HCP.bin[3,,1],HCP.bin[49,,1]))
